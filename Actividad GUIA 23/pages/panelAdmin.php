    ...
    if ($nombre === '' || $descripcion === '') {
        ...
        if (!empty($_FILES['foto']['name'])) {
            ...
            if (is_uploaded_file($_FILES['foto']['tmp_name'])) {
                $qu = time();
                $dest = "../img/{$qu}.{$ext}";
                if (copy($_FILES['foto']['tmp_name'], $dest)) {
...
<body>
    <header class="header">
        ...
        <nav class="nav small"><a href="../index.php">Inicio</a> <a href="logout.php">Salir</a>
        </nav>
    </header>
    <main class="container">
        <?php if ($msg)
            echo '<p class="alert">' . h($msg) . '</p>'; ?>
        ...
        <form class="form" method="post" enctype="multipart/form-data">
            ...
            <div class="row">
                ...
                <textarea name="descripcion" rows="4"
                    required><?= h($edit['descripcion'] ?? '') ?></textarea>
            </div>
            <div class="row2">
                <div>
        ...
        if ($rs) {
            ...
            while ($row = mysqli_fetch_assoc($rs)) {
                ...
                echo '<td><a class="btn secondary" href="panelAdmin.php?action=edit&id=' . (int) $row['id'] . '">Editar</a> ';
                echo '<a class="btn" href="panelAdmin.php?action=del&id=' . (int) $row['id'] . '" onclick="return confirm(\'¿Eliminar?\')">Eliminar</a></td>';
                echo '</tr>';
            }
            echo '</tbody></table>';
        } else {
            echo '<p class="alert">' . h(mysqli_error($con)) . '</p>';
        }
        ?>
    </main>
</body>
